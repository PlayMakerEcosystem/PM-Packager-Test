//-----------------------------------------------------------------------
// <copyright file="EcosystemBrowser.cs" company="Hutong Games LLC">
// Copyright (c) Hutong Games LLC. All rights reserved.
// </copyright>
// <author name="Jean Fabre"</author>
//-----------------------------------------------------------------------

#if UNITY_2017 && UNITY_2017_1 || UNITY_2017_2 || UNITY_2017_3 || UNITY_2017_4 || UNITY_2018_1_OR_NEWER || UNITY_2019_1_OR_NEWER || UNITY_2020_1_OR_NEWER
#define ECOSYSTEM_CUSTOM_WWW
#else
#undef ECOSYSTEM_CUSTOM_WWW
#endif
...
namespace com.hutonggames.playmakereditor.addons.ecosystem
{
    public class EcosystemBrowser : EditorWindow
    {
        ...
        public static string
            PathToVersionInfoSource; // "Assets/Net.Fabrejean/Editor/PlayMaker/Ecosystem/VersionInfo.json";
        ...
        enum PlayMakerEcosystemFilters
        {
            Actions,
            Templates,
            Samples,
        ...
        private enum PlayMakerEcosystemRepositoryMasks
        {
            Unity3x,
            Unity4x,
            Unity5x,
        ...
        */
        ...
        void OnGUI_ProjectScanner()
        {
            ...
            else
            {
                ...
                {
                    MyUtils.OnGUILayout_BeginHorizontalCentered();
                    GUILayout.Label(ProjectScanner.instance.foundAssetsCountInProject + " Assets found from " +
                                    ProjectScanner.instance.AssetsCount + " known Assets.");
                    MyUtils.OnGUILayout_EndHorizontalCentered();
                    /*
                    MyUtils.OnGUILayout_BeginHorizontalCentered();
        ...
        void OnGUI_ScannerItem(AssetItem item, int rowIndex)
        {
            ...
            {
                case "PlayMaker":
                    itemLabelSkin = "Label Round Green Small";
                    break;
                case "AssetStore":
                    itemLabelSkin = "Label Round Violet Small";
                    break;
                case "Addon":
                    itemLabelSkin = "Label Round Cyan Small";
                    break;
                default:
                    itemLabelSkin = "Label Round Blue Small";
                    break;
            }
        ...
        void OnGUI_SetUpSkin()
        {
            ...
            if (bg != null)
            {
                ...
                GUI.DrawTextureWithTexCoords(new Rect(0, 0, position.width, position.height), bg,
                    new Rect(0, 0, position.width / bg.width, position.height / bg.height));
            }
        ...
        void OnGUI_Main()
        {
            ...
            {
                GUILayout.Label(
                    "Application is playing. It saves performances to not process anything during playback.");
                return;
            }
...
*/
        ...
        void OnGUI()
        {
            ...
            if (isCompiling != EditorApplication.isCompiling)
            {
                ...
                if (!isCompiling)
                {
                    ...
                    if (!string.IsNullOrEmpty(PathToVersionInfoSource))
                    {
            ...
            else if (!string.IsNullOrEmpty(PathToVersionInfoSource) && string.IsNullOrEmpty(CurrentVersionAsString))
            {
                CurrentVersion = MyUtils.UpdateVersion(PathToVersionInfoSource);
                CurrentVersionAsString = CurrentVersion.ToShortString();
            }
        ...
        void OnGUI_ToolStrip()
        {
            ...
            if (GUILayout.Button("Settings", EditorStyles.toolbarDropDown))
            {
                ...
                toolsMenu.AddItem(new GUIContent("Show Item Unity version"), DisplayItemUnityVersion_on,
                    OnTools_ToggleDisplayItemUnityVersion);
        ...
        void OnGUI_ToolBar()
        {
            ...
            if (wwwSearch != null)
            {
                ...
                if (GUILayout.Button(new GUIContent(GUI.skin.FindStyle("Cross Icon").normal.background),
                    "Button Round Small"))
                {
                    wwwSearch.Dispose();
                    wwwSearch = null;
        ...
        void OnGUI_FilterButtonOld()
        {
            ...
            if (searchFilters != null && searchFilters.Count > 0 &&
                searchFilters.Count != PlayMakerEcosystemFiltersLength)
            {
    ...
    */
        ...
        void OnGUI_BottomPanel()
        {
            ...
            if (ShowActionDetails && SelectedIndex >= 0 && resultItems != null && SelectedIndex < resultItems.Length)
            {
                ...
                {
                    GUILayout.Label((string) item.DocDescription["description"], "Label Small");
                }
                else
                {
                    /*
                    GUILayout.BeginHorizontal();
                    GUILayout.FlexibleSpace();
                    GUILayout.Label("Documentation unavailable","Label FinePrint");
                    GUILayout.FlexibleSpace();
                    GUILayout.EndHorizontal();
                    */
                }
                ...
                if (item.DocumentationImageStatus == Item.AsynchContentStatus.Available)
                {
                    if (loadingImage || currentDocImageIndex != SelectedIndex)
                    {
                        ...
                        else
                        {
                            ...
                            DocumentationImageWidth = item.DocumentationImage.width;
                            if (item.DocumentationImage.width > 350)
                            {
                                DocumentationImageHeight = DocumentationImageHeight / 2;
                                DocumentationImageWidth = DocumentationImageWidth / 2;
                            }
                    ...
                    if (DocumentationImageHeight < 250)
                    {
                        ...
                        GUILayout.Label(_docImageContent,
                            GUILayout.Width(DocumentationImageWidth),
                            GUILayout.Height(DocumentationImageHeight));
                        GUILayout.FlexibleSpace();
                        GUILayout.EndHorizontal();
                    }
                    else
                ...
                else if (item.DocumentationImageStatus == Item.AsynchContentStatus.Downloading)
                {
                    GUILayout.BeginHorizontal();
                    GUILayout.FlexibleSpace();
                    GUILayout.Label("Downloading...", "Label FinePrint");
                    GUILayout.FlexibleSpace();
                    GUILayout.EndHorizontal();
                }
                else if (item.DocumentationImageStatus == Item.AsynchContentStatus.Unavailable)
                {
                    GUILayout.BeginHorizontal();
                    GUILayout.FlexibleSpace();
                    GUILayout.Label("Screenshot unavailable", "Label FinePrint");
                    GUILayout.FlexibleSpace();
                    GUILayout.EndHorizontal();
                }
        ...
        //    if (Event.current.type == EventType.Repaint)
          //      rowsArea = new Rect[resultItems.Length];
            ...
            if (mouseOverRowIndex != -1 && rowsArea != null && rowsArea.Length > mouseOverRowIndex &&
                resultItems != null && resultItems.Length > mouseOverRowIndex)
                {
                    GUILayout.BeginArea(new Rect(rowsArea[mouseOverRowIndex].x + 4, rowsArea[mouseOverRowIndex].y + 4,
                        rowsArea[mouseOverRowIndex].width - 8, rowsArea[mouseOverRowIndex].height - 8));
        ...
        void OnGUI_SearchItem(Item item, int rowIndex)
        {
            ...
            if (
                !item.RawData
                    .ContainsKey(
                        "projectPath")) // Cache the project path to avoid process the same thing over and over again.
            {
                forceloading = true;
            ...
            string _RawName = (string) item.RawData["name"];

            if (_type == "Package" && _RawName.EndsWith("action.txt"))
            {
                _type = "PackagedAction";
                _label = "Action";
            }
            ...
            GUILayout.Label(_label, itemLabelSkin, GUILayout.Width(61));
            GUI.backgroundColor = Color.white;
...
#if ECOSYSTEM_CUSTOM_WWW
            if (Debug_on) Debug.Log("Using custom WWW");
            wwwSearch = new HutongGames.PlayMaker.Ecosystem.Utils.WWW(url);
#else
				if (Debug_on) Debug.Log("Using regular WWW");
        ...
        void ParseSearchResult(string jsonString)
        {
            ...
            try
            {
                ...
                {
                    _lastError = "json content is null. Please search again, it's likely a connection issue :" +
                                 wwwSearch.url + " ->" + jsonString;
            ...
            if (searchResultHash.ContainsKey("message"))
            {
                ...
                {
                    _lastError =
                        "API rate limit (20 requests per minute) exceeded. Please wait one minute to search again";
                }
                else
                {
        ...
        void DeleteAsset(string assetPath)
        {
            ...
            string
                guid = null; //AssetDatabase.AssetPathToGUID(assetPath); // doesn't work properly, need to pass the right path
            ...
            {
                if (Debug_on)
                    Debug.Log("we found a guid -> " + guid + " assetPath: " + AssetDatabase.GUIDToAssetPath(guid));
        ...
        {
            if (Debug_on)
                Debug.Log("ProceedWithImport for " + url + " rawContent:" + rawContent + " rawBytes:" + rawBytes);
            ...
            // is it a template?
            if (rawContent.Contains("__TEMPLATE__") || rawContent.Contains("__SAMPLE__") ||
                rawContent.Contains("__PACKAGE__"))
            {
                ...
                if (_meta.ContainsKey("unitypackage"))
                {
                    ...
                    string _packageUrl = __REST_URL_BASE__ + "download?repository=" + _repositoryPath + "&file=" +
                                         _packagePath;
                ...
                if (_dependancies != null)
                {
                    foreach (object dScript in _dependancies)
                    {
                        ...
                        if (_dscript.StartsWith("Assets/")) // we are local to the file repository
                        {
                            ...
                            dscripturl = __REST_URL_BASE__ + "download?repository=" +
                                         Uri.EscapeDataString(repositoryPath) + "&file=" +
                                         Uri.EscapeDataString(_dscript);
                        }
                        else if (_dscript.StartsWith("http"))
                        {
                            // we have a straight url
            ...
            if (url.Contains(".unitypackage"))
            {
                ...
                string unityPackageTempFile = Application.dataPath.Substring(0, Application.dataPath.Length - 6) +
                                              "Temp/PlayMakerEcosystem.downloaded.unityPackage";
            ...
            {
                if (Debug_on) Debug.LogError("Writing to file failed, no content found");
            }
        ...
        void PostPonBrowserUpdate()
        {
            ...
            EditorPrefs.SetString(__namespace__ + "." + LastUpdateBannerVersion_prefKey,
                ECO_BrowserVersion.ToShortString());
    ...
    */
        ...
        void GetEditorPrefs()
        {
            ...
            DisplayItemUnityVersion_on =
                EditorPrefs.GetBool(__namespace__ + "." + DisplayItemUnityVersion_on_prefKey, false);
            ...
            string _lastUpdateBannerVersion_pref =
                EditorPrefs.GetString(__namespace__ + LastUpdateBannerVersion_prefKey);
